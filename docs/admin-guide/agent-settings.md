...
**General Settings - MSIX**

![General MSIX Settings](images/agent-settings/agent-settings-01.png)

**General Settings - APP-V**

![General APP-V Settings](images/agent-settings/agent-settings-02.png)

**General Settings - App Control**

![General App Control Settings](images/agent-settings/agent-settings-03.png)

**General Settings - User Settings**

![General User Settings Settings](images/agent-settings/agent-settings-04.png)
---
...
| Run refresh cycle to update the cache | Invoke the refresh cycle at machine start to pre-cache packages from the content store(s) (when enabled for the content store). |
| Detect image state (Citrix PVS/MCS integration) | When the machine starts, the agent can detect the image state (read-only or read/write) and make smart decisions about how to deploy packages. Other agent settings are updated automatically when you enable this feature. There are 2 supported scenarios: **Scenario 1:** Private mode: Stop Refresh cycle. Read-only mode: Deploy packages. Often used for non-persistent RDS environments; you can redirect the cache to a persistent drive. **Scenario 2:** Private mode: Deploy packages in the cache. Read-only mode: Deploy new packages using optimized delivery (App-V SCS mode or MSIX App Attach). Often used for non-persistent VDI environments. When the image is opened for updates, the agent pre-caches all packages from the content stores. When running in read-only mode, the agent uses SCS mode (App-V) or App Attach (MSIX) to prevent write cache pollution. |
| Clear cache | Instructs the agent to clear the cache at machine start before packages from the content store(s) are pre-cached. This is an advanced feature: it first removes packages and then removes any leftovers in the cache to make sure it is empty. Often used for non-persistent environments where the cache is redirected to another drive. |

**Machine Start Actions - MSIX**

![Machine Start Actions - MSIX](images/agent-settings/agent-settings-05.png)

**Machine Start Actions - App-V**

![Machine Start Actions - App-V](images/agent-settings/agent-settings-06.png)

---

## Refresh settings

Here you can configure the refresh settings.
...
| Pre-cache packages from the content stores with the pre-cache option enabled | A mandatory setting. You can control whether to pre-cache packages using the checkbox next to the content store in the machine group configuration window. |
...
| Only process publishing tasks for packages on content store(s) configured for the machine group | Publishing tasks are executed for users on machines regardless of where the package is stored by default. With this setting, only publishing tasks are executed for packages that are on one of the content store(s) configured for the machine group. |
| Remove packages from the cache that are no longer on the configured content store(s) | Removes packages from the cache when they are no longer found on one of the configured content store(s). Especially useful for persistent environments to keep the cache clean and up to date. Often not needed when clear cache at machine start is enabled. |
...
**Refresh settings - MSIX**

![Refresh settings - MSIX](images/agent-settings/agent-settings-07.png)

**Refresh settings - App-V**

![Refresh settings - App-V](images/agent-settings/agent-settings-08.png)

**Refresh settings - User Settings**

![Refresh settings - User Settings](images/agent-settings/agent-settings-09.png)
...
**Client Settings - MSIX**

![Client Settings - MSIX](images/agent-settings/agent-settings-10.png)

**Client Settings - App-V**

![Client Settings - App-V](images/agent-settings/agent-settings-11.png)
...
![Inventory Settings](images/agent-settings/agent-settings-12.png)
...
| Setting | Description |
|---------|-------------|
| Exclude the following directory names on the configured content store(s) | A list of folder names skipped by the agent when performing pre-cache or balance cache procedures. By default, folders containing "backup" and "dfsrprivate" are excluded. The list is separated with a semicolon (;). |
...
| Agent content cache location | For the pre-cache and balance cache methods, the content store is scanned for changes. The content cache keeps a cache of the content on the share(s) so the number of reads is reduced. In non-persistent scenarios this location can be configured to a persistent drive like `D:\AppVentiXCache`. |
...
**Advanced Settings - MSIX**

![Advanced Settings - MSIX](images/agent-settings/agent-settings-13.png)

**Advanced Settings - App-V**

![Advanced Settings - App-V](images/agent-settings/agent-settings-14.png)