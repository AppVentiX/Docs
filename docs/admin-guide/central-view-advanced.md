...
- **Use SSL for remote machine inventory:** Enables WinRM communication over SSL. Note that you have to deploy a machine certificate to the agent machines in order to use WinRM over SSL. This setting is not used when inventory is done through the configuration share (new default).
...
- **Remote connection (WinRM) time-out value:** The maximum time to wait before timing out a connection to an agent. Increase this value when running inventory over WAN connections. Not used when inventory is done through the configuration share (new default).
